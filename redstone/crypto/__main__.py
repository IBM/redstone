...
def main():
    ...
    ep.add_argument("--key-crns",
        help=("Space separated list of CRNs of KMS Keys to use during encryption. "
              "At least one CRN must be specified here or by setting %s" % key_crn_env_var))
    ep.add_argument("infile", type=argparse.FileType("rb"),
        help="Input file to encrypt. Output will be written to stdout. Use '-' to read from stdin.")

    dp = subp.add_parser("decrypt")
    dp.add_argument("infile", type=argparse.FileType("rb"),
    ...
    if args.action == "encrypt":
        ...
        if not key_crns:
            print("No key CRNs specified. Please pass the CRNs with --key-crns or set '%s'" % key_crn_env_var, file=sys.stderr)
            return 1
        ...
        message, _ = crypto.encrypt(data, key_crns=key_crns, aad="redstone.crypto.__main__")
